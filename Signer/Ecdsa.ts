...
export class Ecdsa extends Base {
	...
	constructor(
		...
		if (publicKey) {
			...
			this.publicKey = globalThis.crypto.subtle.importKey(
				"spki",
				publicKey,
		...
		if (privateKey) {
			...
			this.privateKey = globalThis.crypto.subtle.importKey(
				"pkcs8",
				privateKey,
				{ name: "ECDSA", namedCurve: hash.replace("SHA", "P") == "P-512" ? "P-521" : hash.replace("SHA", "P") },
				true,
	...
	protected async signBinary(data: Uint8Array): Promise<Uint8Array> {
		return new Uint8Array(
			this.privateKey
				? await globalThis.crypto.subtle.sign({ name: "ECDSA", hash: { name: this.hash } }, await this.privateKey, data)
				: new ArrayBuffer(0)
		)
	}
	protected async verifyBinary(data: Uint8Array, signature: Uint8Array): Promise<boolean> {
		return (
			...
			globalThis.crypto.subtle.verify(
				{ name: "ECDSA", hash: { name: this.hash } },

...
describe("Algorithm", () => {
	it("generate AES CBC 256", async () => {
		const algorithm = cryptly.Encrypter.Aes.cbc(256)
		const key = await algorithm.export()
		expect(key).toHaveLength(43)
	})
	it("generate AES GCM 256", async () => {
		const algorithm = cryptly.Encrypter.Aes.gcm(256)
		const key = await algorithm.export()
		expect(key).toHaveLength(43)
	})
	...
	it("encrypt & decrypt AES GCM", async () => {
		const algorithm = cryptly.Encrypter.Aes.gcm("25ji7n8YAKwPvyWrtR3lUUbxABzAHA2zzdIG_Zb13Iw")
		const encrypted = await algorithm.encrypt("The meaning of life, the universe and everything.")
		expect(encrypted.salt).toHaveLength(22)
	...
	it("encrypt & decrypt AES CBC with fixed salt", async () => {
		const algorithm = cryptly.Encrypter.Aes.cbc("VaVdZS5E6s9oEdLonNVTgSKv5zAW0LNvjBSdrmnEBS8")
	...
	it("split keys", async () => {
		...
		const imported = await cryptly.Encrypter.Aes.cbc(keys).export()
	...
	it("export left/right key", async () => {
		...
		const key = await algorithm.export()
		expect(key).toHaveLength(43)
		const left = cryptly.Encrypter.Aes.random(256)
	...
	it("random", async () => {
		const random1 = cryptly.Encrypter.Aes.random(256)
		const random2 = cryptly.Encrypter.Aes.random(256, 2)
		const random3 = cryptly.Encrypter.Aes.random(256, 3)

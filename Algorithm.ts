...
export class Algorithm {
	...
	async export(parts: number = 1): Promise<string | string[]> {
	...
	static aesCbc(key: 256 | string | string[]): Algorithm {
		return new Algorithm(
			typeof key == "number" ? crypto.subtle.generateKey({ name: "AES-CBC", length: key }, true, ["encrypt", "decrypt"]) :
			crypto.subtle.importKey("raw",
				Array.isArray(key) ? Algorithm.reduceKeys(key.map(k => Base64.decode(k, "url"))) :
				Base64.decode(key, "url"),
			"AES-CBC", true, ["encrypt", "decrypt"]))
	}
	private static generateRandomKeys(length: number, parts: number): Uint8Array[] {
		return parts > 0 ? [crypto.getRandomValues(new Uint8Array(length)), ...this.generateRandomKeys(length, parts - 1)] : []
	}

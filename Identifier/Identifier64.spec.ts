...
describe("Identifier64", () => {
	it("generate is", () => expect(cryptly.Identifier.is(cryptly.Identifier.generate(64))).toEqual(true))
	it("generate is length 64", () => expect(cryptly.Identifier.is(cryptly.Identifier.generate(64), 64)).toEqual(true))
	it("generate is not length 64", () =>
		expect(cryptly.Identifier.is(cryptly.Identifier.generate(32), 64)).toEqual(false))
	it("generate length", () => expect(cryptly.Identifier.generate(64)).toHaveLength(64))
	it.each([[1691418818480, /^---0XS0exv[\w\d-_]{54}$/]])(`generate ordered w/ prefix %s`, (prefix, result) =>
		expect(cryptly.Identifier.generate(64, "ordered", prefix)).toMatch(result)
	)
	it.each([[1691418818480, /^zzzySXyK13z[\w\d-_]{53}$/]])(`generate reversed w/ prefix %s`, (prefix, result) =>
		expect(cryptly.Identifier.generate(64, "reversed", prefix)).toMatch(result)
	)
	const data = [
		{
	...
	it.each(data)(`is %s`, ({ identifier }) => expect(cryptly.Identifier.is(identifier, 64)).toBeTruthy())
	...
	it.each([
		...
		expect(
			cryptly.Identifier.generate(64, "ordered", time + left) < cryptly.Identifier.generate(64, "ordered", time + right)
		).toEqual(true)
	)
	it.each([
		[0, 1],
		...
		expect(
			cryptly.Identifier.generate(64, "reversed", time + left) >
				cryptly.Identifier.generate(64, "reversed", time + right)
		).toEqual(true)
	)
})
